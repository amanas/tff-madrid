...
data.4000 <- 4000 %>% tff.get.parsed.metrics(d = .) %>% tff.clean %>% tff.pad %>% 
  na.interpolation %>% .[,c("fecha","carga")]
...
```{r acf-pacf-4000, fig.cap="Original, ACF y PACF para los valores de carga del dispositivo 4.000 a finales de Septiembre de 2018"}
ggtsdisplay(data.4000 %>% tail(2000) %>% tff.as.ts, main='Curvas ACF y PACF de la serie 4.000')
```
...
```{r arima-4000, fig.cap='Pronóstico del flujo de carga para el terminal 4.000 con ARIMA'}
"select * from experiments_new where d=4000 and name like '%auto.arima%'" %>% dbGetQuery(tff.get.conn(), .) %>% 
    mutate(name='Auto ARIMA') %>% tff.plot.exps + xlab("Fecha") + ylab("% carga") 
```
